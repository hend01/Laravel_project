...
<div class="top-bar">
 ...
 <h2 class="intro-y text-lg font-medium mt-10" style="background-color:#1e40af;color:white;border-radius: 8px;">
            <span style="margin-left: 11px;"> Modifier un entretien </span>
         </h2>    <!-- Display a table with the list of entretiens -->
   

    <form method="POST" action="{{ route('entretiens.update', $entretien->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="car_id" name="car_id" required>
                @foreach($cars as $car)
                    <option value="{{ $car->id }}" {{ $car->id == $entretien->car_id ? 'selected' : '' }}>{{ $car->nom }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="kilometrage" name="kilometrage" value="{{ $entretien->kilometrage }}" required>
        </div>
        <div class="mb-3">
            <label for="date_entretien" class="form-label">Date d'Entretien</label>
            <input type="date" class="form-control" id="date_entretien" name="date_entretien" value="{{ $entretien->date_entretien }}" required>
        </div>
        <div class="mb-3">
            <label for="description" class="form-label">Description</label>
            <textarea class="form-control" id="description" name="description" rows="3" required>{{ $entretien->description }}</textarea>
        </div>
