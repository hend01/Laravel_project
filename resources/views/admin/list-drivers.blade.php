...
<div class="content">
    ...
    <h2 class="intro-y text-lg font-medium mt-10" style="background-color:#1e40af;color:white;border-radius: 8px;">
        <span style="margin-left: 11px;"> Drivers list </span>
    </h2>
<a href="{{ route('drivers.addDriver') }}" class="text-blue-500 add-driver-link block" style="position: absolute; top: 212px; left: 290px; z-index: 9999; font-weight: bold; color: #1e40af;">Add new driver +</a>
        ...
        <tbody>
            @foreach($drivers as $driver)
                <tr>
                    ...
                    <td>
                        <form action="{{ route('drivers.deleteDriver', $driver->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" style="margin-bottom:4px">Delete</button>
                        </form>
                    
                        <a href="{{ route('drivers.edit', $driver->id) }}"style="width:66px" class="btn btn-primary">Edit</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
