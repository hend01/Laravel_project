...
<div class="content">
    ...
    <a href="{{ route('event.create') }}" class="text-blue-500 add-car-link block" style="position: absolute; top: 212px; left: 290px; z-index: 9999; font-weight: bold; color: #1e40af;">Add new event +</a>
            ...
            <tbody>
                @foreach($events as $event)
                    <tr>
                        ...
                        <td>{{  $event->name }}</td>
                        <td>{{ $event->produit_number }}</td>
                        <td>{{ $event->description }}</td>
                        <td>{{ $event->event_type }}</td>
                    
                        <td>
                            <form action="{{ url("/event/delete/$event->id")}}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" style="margin-bottom: 4px">Delete</button>
                            </form>
                        
                            <a href="{{ route('events.update', $event->id) }}" style="width:66px" class="btn btn-primary">Edit</a>
                            @csrf
                            </form>
                        </td>
